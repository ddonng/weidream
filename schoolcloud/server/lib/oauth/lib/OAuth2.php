...
class OAuth2 {
	 ...
	 */
	const DEFAULT_ACCESS_TOKEN_LIFETIME = 43200;//12h
	const DEFAULT_REFRESH_TOKEN_LIFETIME = 1296000;//15day
	...
	protected function setDefaultOptions() {
		$this->conf = array(
			...
			self::CONFIG_ENFORCE_INPUT_REDIRECT => TRUE,//直接在这里修改了
	...
	public function verifyAccessToken($token_param, $scope = NULL) {
		...
		//与api.class.php配合，这里原来oauth2-php提供的错误处理全部更改为 return 错误字段，再匹配
		...
		if (!isset($token["expires"]) || !isset($token["client_id"])) {
			...
			return 'Malformed token (missing "expires" or "client_id")';
		}
	...
	public function grantAccessToken(array $inputData = NULL, array $authHeaders = NULL) {
		...
		switch ($input["grant_type"]) {
			...
			case self::GRANT_TYPE_REFRESH_TOKEN:
				...
				//获取client数据,确保redirect_uri有效
				$client_detail = $this->storage->getClientDetails($inputData["client_id"]);
				if ($client_detail["redirect_uri"] && $input["redirect_uri"] && !$this->validateRedirectUri($input["redirect_uri"], $client_detail["redirect_uri"])) {
		...
		if($input["grant_type"]===self::GRANT_TYPE_CLIENT_CREDENTIALS)
		{
		...
		//if($user_id) $result["query"]["user_id"]=$user_id;

		header("HTTP/1.1 " . self::HTTP_FOUND);
		header("Location: " . $client_detail["redirect_uri"]."?".http_build_query($token));
		exit();
	}
	...
	public function getAuthorizeParams(array $inputData = NULL) {
		...
		//必须保证都是数组。+为前面覆盖后面，array_merge为后面覆盖前面
		return ($input + $stored);
	...
	protected function genAccessToken() {
		$tokenLen = 40;
		/**SAE不支持,报错
		* Warning: file_exists() [function.file-exists]: open_basedir restriction in effect. File(/dev/urandom) is not within the allowed path(s): 
		* (C:\sae\emulation\;C:\sae\wwwroot\;C:\Windows\fonts\;C:\sae\storage\;C:\Windows\;C:\Users\;C:\Users\ADMINI~1\AppData\Local\Temp") in 
		* C:\sae\wwwroot\schoolcloud\1\server\lib\oauth\lib\OAuth2.php on line 1075
		*/
